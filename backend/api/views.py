...
from users.models import (
    ...
    StatusChoices,
    RoleChoices,
)  # Import UserProfile and Choices
...
class AdminUserViewSet(viewsets.ModelViewSet):  # Changed from ReadOnlyModelViewSet
    ...
    def approve_user(self, request, pk=None):
        ...
        if profile.approval_status != StatusChoices.PENDING:
            return Response(
                {
                    "detail": f"User status is already {profile.get_approval_status_display()}. Cannot approve."
                },
    ...
    def set_user_status(self, request, pk=None):
        ...
        if new_status not in valid_statuses:
            return Response(
                {
                    "detail": f'Invalid status provided. Use "{StatusChoices.ACTIVE}" or "{StatusChoices.INACTIVE}".'
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
