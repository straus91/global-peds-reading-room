...
class MasterTemplateSectionAdmin(admin.ModelAdmin):
    ...
    def master_template_link(self, obj):
        if obj.master_template:
            link = reverse(
                "admin:cases_mastertemplate_change", args=[obj.master_template.id]
            )
...
class CaseTemplateSectionContentInline(admin.TabularInline):
    ...
    formfield_overrides = {
        models.TextField: {
            "widget": admin.widgets.AdminTextareaWidget(attrs={"rows": 2, "cols": 40})
        },
    }
...
class CaseTemplateAdmin(admin.ModelAdmin):
    ...
    def case_link(self, obj):
        if obj.case:
            ...
            display_name = (
                obj.case.case_identifier if obj.case.case_identifier else obj.case.title
            )
...
class CaseTemplateSectionContentAdmin(admin.ModelAdmin):
    ...
    list_display = (
        "id",
        "case_template_link",
        "master_section_name_admin",
        "content_preview",
        "key_concepts_text_preview",
    )
    list_filter = (
        "case_template__case__title",
        "case_template__case__case_identifier",
        "master_section__name",
    )
    search_fields = (
        "content",
        "master_section__name",
    ...
    def key_concepts_text_preview(self, obj):
        if obj.key_concepts_text:
            return (
                (obj.key_concepts_text[:75] + "...")
                if len(obj.key_concepts_text) > 75
                else obj.key_concepts_text
            )
        return "-"
    ...
    def case_template_link(self, obj):
        if obj.case_template:
            link = reverse(
                "admin:cases_casetemplate_change", args=[obj.case_template.id]
            )
...
class CaseAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        ...
        "difficulty",
        "created_by",
        "master_template__name",
        "patient_sex",  # Added patient_sex
    )
    search_fields = (
        "title",
        "case_identifier",
        "clinical_history",
        "key_findings",  # Added case_identifier
        "diagnosis",
        "discussion",
    ...
    fieldsets = (
        (None, {"fields": ("case_identifier", "title", "status", "published_at")}),
        (
...
class ReportAdmin(admin.ModelAdmin):
    ...
    def user_link(self, obj):
        if obj.user:
            ...
            try:
                link = reverse(
                    f"admin:{user_model_meta.app_label}_{user_model_meta.model_name}_change",
                    args=[obj.user.id],
                )
            except Exception:  # Catch URL reverse errors
...
class AIFeedbackRatingAdmin(admin.ModelAdmin):
    ...
    ):  # Ratings are immutable via admin
        return False
