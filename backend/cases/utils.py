...
)  # Required for type hinting if used, or direct access
...
def generate_report_comparison_summary(
    ...
    expert_sections_map = {
        esc.master_section_id: {
            "content": esc.content,
            "key_concepts": (
                [kc.strip() for kc in esc.key_concepts_text.split(";") if kc.strip()]
                if esc.key_concepts_text
                else []
            ),
        }
    ...
    for user_section in user_report_structured_content:
        ...
        section_comp = {
            ...
            "text_comparison_status": (
                "Expert Section Missing"
                if not expert_section_data
        ...
        if expert_section_data:
            ...
            section_comp["expert_content_preview"] = (
                (expert_content[:100] + "...")
                if len(expert_content) > 100
                else expert_content
            )
            ...
            if expert_key_concepts:
                section_comp["key_concepts_status"] = (
                    "All Addressed"  # Assume all addressed initially
                )
                ...
                section_comp["key_concepts_status"] = (
                    "No Key Concepts Defined by Expert for this Section"
                )
    ...
    if case_diagnosis_text:
        ...
        if not user_impression_content:
            comparison_summary["overall_diagnosis_comparison"][
                "status"
            ] = "User Impression Missing"
            comparison_summary["overall_diagnosis_comparison"][
                "detail"
            ] = f"User did not provide an impression. Expert diagnosis is: '{case_diagnosis_text}'."
        elif case_diagnosis_lower in user_impression_content:
            comparison_summary["overall_diagnosis_comparison"][
                "status"
            ] = "Aligns with Expert Diagnosis"
            comparison_summary["overall_diagnosis_comparison"][
                "detail"
            ] = f"User's impression mentions the expert diagnosis: '{case_diagnosis_text}'."
        else:
            comparison_summary["overall_diagnosis_comparison"][
                "status"
            ] = "Deviates from Expert Diagnosis"
            comparison_summary["overall_diagnosis_comparison"][
                "detail"
            ] = f"User's impression does not clearly state the expert diagnosis ('{case_diagnosis_text}'). User Impression: '{user_impression_content[:150]}...'."
    else:
        comparison_summary["overall_diagnosis_comparison"][
            "status"
        ] = "Expert Diagnosis Not Provided for Case"
        comparison_summary["overall_diagnosis_comparison"][
            "detail"
        ] = "No overall expert diagnosis was provided for this case to compare against."
