...
from .models import (
    ...
    RoleChoices,
    StatusChoices,
)  # Import your UserProfile model and choices
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    approval_status = serializers.CharField(
        read_only=True
    )  # Or simply serializers.CharField() if you might write to it elsewhere
...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    role = serializers.ChoiceField(
        choices=RoleChoices.choices, write_only=True
    ...
    def validate(self, attrs):
        ...
        if (
            User.objects.filter(email__iexact=attrs["email"]).exists()
            or User.objects.filter(username__iexact=attrs["email"]).exists()
        ):
            raise serializers.ValidationError(
